...
def open_profile_window():
    ...
    save_button= ctk.CTkButton(new_window, text="Save", command= save_profile)
...
# LINKE SEITE: TAGAUSWAHL, ANALYSEN UND GERICHTE
...
dishes_label = ctk.CTkLabel(left_frame, text="Today's Dishes", font=("Arial", 22, "bold"))
...
def new_dishes(day):
    ...
    for dish in schedule[chosen_day]:
        button = ctk.CTkButton(dishes_frame, text=dish["name"], height=50, 
                               fg_color="#A8C3A0",
                               hover_color="#8FAE87",
                               text_color="black",
...
def daily_analytics():
    ...
    for dish, count in orders_count.items(): 
        text_l += f"{dish}: {count}\n"
    details_title.configure(text=f"Daily Analytics: {day}")
...
weekday = ctk.CTkOptionMenu(left_frame, 
...
daily_analytics_button = ctk.CTkButton(left_frame, text="Daily analytics", 
                                 fg_color="#A8C3A0", 
                                 hover_color="#8FAE87", 
                                 text_color="black", 
                                 command=daily_analytics)
weekly_analytics_button = ctk.CTkButton(left_frame, text="Weekly analytics",
                                 fg_color="#A8C3A0", 
                                 hover_color="#8FAE87", 
                                 text_color="black", 
...
def show_dish(dish):
    ...
    """
        if "goal" in user_profile:
            protein_percent = round(dish["protein"] / (float(user_profile["weight"]) * 2) * 100)
...
order_button = ctk.CTkButton(right_frame, text="Order", command=order_dish, 
                             fg_color="#A8C3A0",
                             hover_color="#8FAE87",
                             text_color="black")
...
feedback_label = ctk.CTkLabel(right_frame, text="Rate this dish:", font=("Arial", 18))

feedback_label.pack(pady=10)

slider = ctk.CTkSlider(right_frame, from_=1, to=5, number_of_steps=4)

slider.pack(pady=10)

comment_box = ctk.CTkTextbox(right_frame, width=300, height=120)

comment_box.pack(pady=10)

submit_button = ctk.CTkButton(right_frame, text="Submit Feedback", command=save_feedback, 
                              fg_color="#A8C3A0",
                              hover_color="#8FAE87",
                              text_color="black")
